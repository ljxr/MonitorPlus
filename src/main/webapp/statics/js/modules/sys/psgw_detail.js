...
//		alert("流量");
		...
		types_name=["余氯","pH","浊度"];
		types_unit=["mg/L","","NTU"];		
	}else if(vm.TYPE=="P02" || vm.TYPE=="P03"){//排水管网类型
		console.log("排水管排水管排水管排水管排水管");
		typs=["PTOUT","FLS","FLC","TN","TP","HZD","COD","NH4","HOCL","PH","TUR"];
		types_name=["压力","瞬时流量","累计流量","总氮","总磷","固体浑浊度","COD","氨氮","余氯","pH","浊度"];
		types_unit=["MPa","m³/h","m³","mg/L","mg/L","NTU","mg/L","mg/L","mg/L","","NTU"];		
	}else if(vm.TYPE=="P04"){//液位
	  	typs=["LT"];
		types_name=["液位"];
		types_unit=["m"];		
	}
...
//	}
	...
	console.log(vm.historyData.length,"9999999999999999999999")
	siteId = [];
	...
	for(var i=0;i<vm.historyData.length;i++){
		if(vm.historyData[i].cls=="add"){
			...
			if(vm.historyData[i].type=="PTOUT"){
				...
				his.type="tp";
			}else if(vm.historyData[i].type=="HZD"){
				his.type="hzd";
			}else if(vm.historyData[i].type=="COD"){
				his.type="cod";
			}else if(vm.historyData[i].type=="NH4"){
				his.type="nh4";
			}else if(vm.historyData[i].type=="LT"){
				his.type="lt";
			}
...
//	siteId.push(vm.hisDetail[1].ID+","+vm.hisDetail[1].TP);
	...
	//			"siteId2":siteId[1],
		...
		console.log(vm.hisDetail[0].tit);
		layui.use(['laypage','element','laydate','table'], function(){
			  var laypage = layui.laypage
			  ,element = layui.element//导航的hover效果、二级菜单等功能，需要依赖element模块
			  ,laydate = layui.laydate
