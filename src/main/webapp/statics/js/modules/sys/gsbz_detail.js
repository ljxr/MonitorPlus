var vm = new Vue({
  ...
  methods:{//init() { console.log('test')},
	  ...
	  getIndex2:function(parentID,bzId,bzName){//片区ID，泵站ID，泵站名称
          ...
          console.log(this.current_gsc,vm.current_gsc,"**********************************");
          ...
          if(this.model==1){
        	  ...
        	  $("#area").attr("src","");
        	  $("#jksj").attr("src","jksj.html?ID="+this.current_gsc);
          }else{
        	  $("#area").attr("src","");
          	  $("#jksj").attr("src","");
          }
...
//			    ,contentType:"application/json"
			    ...
			    ,cols: [[ //表头
				      {title: '监测项目', toolbar: '#st1', width:vm.wd}
				      ,{field: vm.hisDetail[0].type, title: '监测值', width:vm.wd}
				      ,{field: 'TIME', title: '采集时间', width:vm.wd}
				    ]]
			   ,id: 'testReload'
			  });
		})
...
//				,contentType:"application/json"
			    ...
			    ,cols: [[ //表头
				      {title: '监测项目', toolbar: '#st2', width:vm.wd}
				      ,{field: vm.hisDetail[1].type, title: '监测值', width:vm.wd}
				      ,{field: 'TIME', title: '采集时间', width:vm.wd}
				    ]]
			   ,id: 'testReload'
			  });
  		...
  		if(key==vm.hisDetail[m].siteId){
  			for(var q=0; q<r[0][key].length; q++){
  				...
  				if(r[0][key][0][type]){
  	  				...
  	  				vm.hisDetail[m].map.push({time:r[0][key][q].TIME,value:r[0][key][q][type]});
  	  				vm.map.push({time:r[0][key][q].TIME,value:r[0][key][q][type]});  					
  				}else if(rr[0][key][0][type]){
  	  				vm.hisDetail[m].time.push(rr[0][key][q].TIME);
  	  				var type=vm.hisDetail[m].type;
  	  				console.log(vm.hisDetail[m].type);
  	  				vm.hisDetail[m].value.push(rr[0][key][q][type]);
  	  				vm.hisDetail[m].map.push({time:rr[0][key][q].TIME,value:rr[0][key][q][type]});
  	  				vm.map.push({time:rr[0][key][q].TIME,value:rr[0][key][q][type]});  	  					
  				}
  		}
  		}
  	  }
...
function lk_show(n){//横向导航栏点击
    ...
    if(n==1){
    	...
    	$("#jksj").attr("src","jksj.html?ID="+vm.current_gsc);
    	$("#area").attr("src","");
    }else{
    	$("#jksj").attr("src","");
    	$("#area").attr("src","");
...
function search_History(){
	...
	$.ajax({
        type: "POST",
        url:  baseURL + "/pipe/data/queryPipePumpSite",
        contentType: "application/json",
        data: JSON.stringify(vm.current_gsc),
        dataType:"json",
        success: function(result){
            ...
            if(result.length){
                if(result[0].siteId){
                	HIS={name:vm.current_name+"出口压力",siteId:vm.current_gsc,type:"PTOUT",cls:"remove",unit:"MPa"};
            		vm.historyData.push(HIS);
                }            	
            }
        }
		  ...
		  success: function(r){
			  ...
			  for(var i=0;i<r.length;i++){//营业所
				  ...
				  for(var m=0; m<r[i].sonList.length; m++){//片区
						  ...
						  success: function(rr){
							  ...
							  for(var i=0;i<rr.length;i++){
								  ...
								  $.ajax({
									    ...
									    success: function(result){
									       console.log(result);
									       fls = result.FLS?result.FLS:"";
									       flc = result.FLC?result.FLC:"";	 
									       console.log(fls,flc);
											  $.ajax({//请求子点位
												    type: "POST",
												    url: baseURL + "sys/site/queryParentSite",
						      ...
						      console.log(vm.bengs);
						      console.log(res.treelist);
							  if(vm.beng.length==27){
								  for(var i=0;i<vm.treeList.length; i++){//营业所
									  for(var m=0;m<vm.treeList[i].treelist.length;m++){//片区
										  for(var n=0;n<vm.beng.length;n++){//泵站
					  ...
					  console.log(res,result.treelist);
...
function backBZ(){
	window.location.href="gsbz.html?id="+vm.current_gsc+"&parentId="+vm.parentId;
}
